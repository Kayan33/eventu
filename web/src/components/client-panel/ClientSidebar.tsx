...
import { Award, Ticket, X } from "lucide-react";
...
export function ClientSidebar({ open, onClose }: ClientSidebarProps) {
  ...
  useEffect(() => {
    if (actor?.type !== "client") return;
    getClient(actor.sub)
      .then((client) => setName(client.name))
      .catch(() => setName(null));
  }, [actor]);

  if (actor?.type !== "client") return null;

  const navItems = [
    { href: "/minhas-inscricoes", label: "Minhas inscrições", icon: Ticket },
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active && "bg-white/10 font-medium text-brand-panel-fg",
                  )}
                >
                  <Icon size={16} />
                  {item.label}
        ...
        <div className="relative border-t border-white/10 p-3.5">
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11px] text-brand-panel-fg-soft">Minha conta</div>
            </div>
          </button>
        </div>
      </aside>
